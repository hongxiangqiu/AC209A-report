...
As our project goes, we successfully improved our test $R^2$ step by step. Our final model is an ensemble model built upon the collabrative filtering regularized regression, matrix factorization, content filtering based ridge and lasso regression and random forest regression (content filtering based). The **meta-regressor is random forest**. Our final model yields $R^2=0.38$ **on the whole dataset excluding the training set**. The test $R^2$ is $0.23$. The difference of two $R^2$ is caused by the `remaining` set. Those are data where each user or restaurant is seldomly seen and therefore, their average ratings have very strong correlation with the actual rating (response). In this case, our content-filtering based models will be 'extremely' accurate since the average rating takes the rating to be predicted into account (think that a user only rate once, then the average rating is just our response, kind of data leakage. But this is not a problem if the user rate for 100 times). However, in real world, if a user or restaurant is so inactive, we won't know their average rating or the average rating is just meaningless at the time we make the prediction (when the average rating calculation does not involve the rating to be predicted). So we think it's more reasonable to say our model will have $R^2 \approx 0.23$ on predicting future data although the whole set $R^2=0.38$.

Since rating has a range of $[1,5]$, we tried to set all values of the final model output below 1 to 1 and all values above 5 to 5 by `min(max(1,x),5)`. However, this gives a same test $R^2$, which means our final model already learned this knowledge.

Detailed project summary can be found [here](model.html#project-summary)
...
Due to sparsity in our dataset, if we use bias withour regularization we can get very good performance on training set. Since many users/restaurants have only few reviews, their biases reveal much information. However, it we want to generalize, we have to apply regualrization. This is demostrated in the huge decrease of score in training and increase in testing when we applied L2 regularization in baseline model. Very similar situation occurs when we try to do matrix factorization. If we use small regularization it's very easy to achieve 99% on the training set, but generalizes extremely bad on test set. We have observed that by using large regularization, we could improve very little on the test set. The extreme sparsity we got in this problem is preventing MF to work well. In future work we could improve this by doing MF seperately for each city/ area which would greatly alleviate the sparsity problem and makes more sense. (we should observe some groups of users frequent some groups of restaurants: if I live in Boston I would frequent Boston restaurants, less Las Vegas ones). 

After item-based KNN turned us down (see details [here](model-exploration.html#item-based-knn)), we moved to content filtering. First we tried neural net, and we only got mediocre results due to limitation of time and knowledge. Then we applied some other models, including Ridge, Lasso, random forest and KNN, they gave us some reasonable $R^2$ scores. In the end, we trained an ensemble model combining our baseline, MF, and content-filtering methods as the final model. The interpretation for the final model result is in [summary part](#summary). 
