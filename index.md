...
After item-based knn turned us down (see details [here](model-exploration.html#item-based-knn)), we moved to content filtering. First we tried neural net, and we only got mediocre results due to limitation of time and knowledge. Then we applied some other models, including Ridge, Lasso, random forest and KNN, they gave us some reasonable r2 scores. In the end we trained a final ensemble model combining our baseline, MF, and content-filtering methods to achieve our final accuracy.
